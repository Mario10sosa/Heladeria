...
namespace Logica
{
    public class FacturaTick
    {
        public class CreaTicket
        {
            ...
            public void AgregaArticulo(string Codigo, string NombreH, string Categoria, int precio, int cant)
            {
                if (cant.ToString().Length <= 3 && precio.ToString("c").Length <= 10) // valida que cant precio y total esten dentro de rango
                {
                    ...
                    if (Codigo.Length > 40)                                 // **********
                    {
                        ...
                        nroEspacios = (11 - Categoria.ToString().Length);
                        espacios = "";
                        ...
                        elementos += espacios + Categoria.ToString();

                        int CaracterActual = 0;// indica en que caracter se quedo
                        for (int Longtext = Codigo.Length; Longtext > 16; Longtext++)
                        {
                            if (bandera == false)
                            {
                                line.AppendLine(Codigo.Substring(CaracterActual, 16) + elementos);
                                bandera = true;
                            }
                            else
                            {
                                line.AppendLine(Codigo.Substring(CaracterActual, 16));

                            }
                            CaracterActual += 16;
                        }
                        line.AppendLine(Codigo.Substring(CaracterActual, Codigo.Length - CaracterActual));


                    }
                    else
                    {
                        for (int i = 0; i < (16 - Codigo.Length); i++)
                        {
                            espacios += " ";

                        }
                        elementos = Codigo + espacios;
                        nroEspacios = (3 - cant.ToString().Length);
                        espacios = "";
                        for (int i = 0; i < nroEspacios; i++)
                        {
